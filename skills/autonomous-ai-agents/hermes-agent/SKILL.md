...
Photon/iMessage verification notes: `hermes send --to photon ...` is not a reliable standalone smoke test because Photon sends require the running gateway sidecar token; use `hermes photon status` + gateway logs for connection health, and for an end-to-end inbound test on macOS send an iMessage to the assigned Photon line (from `hermes photon status` / `auth.json`) with `imsg send --to <assigned_line> --text "..."`, then confirm gateway logs show `inbound message: platform=photon`, `response ready`, and `[Photon] Sending response`.
